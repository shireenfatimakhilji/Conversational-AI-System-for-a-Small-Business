...
Tests 6 scenarios:
  ...
  5. Weather + Off-Topic  — weather support plus unrelated query handling
  6. Delivery Estimation — customer asks when their order will arrive
...
TEST_CASES = [
    ...
    {
        "id": 5,
        "name": "Weather + Off-Topic Handling",
        "description": "Customer asks weather first (supported), then unrelated question; bot should still guide back to order.",
        "turns": [
            "hello",
        ...
        "expect_in_last": ["thank"],
        # Bot should handle weather and keep order flow alive after off-topic input
        "expect_in_any": ["weather", "delivery", "crochet"],
    },
    {
        "id": 6,
        "name": "Delivery Estimation — Calendar Tool",  # <-- added
        "description": "Customer asks when a custom order will arrive; bot should use the calendar tool and reply with an estimated date.",
        "turns": [
            "hello",
            "When will my order arrive if I place it today on 2026-04-20 and it takes 10 days to make?",
            "Ok thank you, I'll go ahead and order",
        ],
        "expect_in_last": [],
        # Bot must mention the delivery date and the long-order note somewhere in the conversation
        "expect_in_any": ["2026-04-30", "longer"],
    },
]
