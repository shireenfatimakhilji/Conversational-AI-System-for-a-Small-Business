...
class SessionStore:
    ...
    def create(self) -> str:
        """Creates a new session, returns its unique ID. Enforces max users."""
        with self._lock:
            # Check if we are already at capacity
            if len(self._sessions) >= self.MAX_USERS:
                raise ValueError("Server is full! Maximum of 4 concurrent users allowed.")
